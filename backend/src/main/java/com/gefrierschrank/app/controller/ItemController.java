...
public class ItemController {
    ...
    public ResponseEntity<List<ItemDto>> getExpiringSoonItems(
            @Parameter(description = "Days ahead to check for expiry") @RequestParam(defaultValue = "7") int days,
            Authentication authentication) {
    ...
    public ResponseEntity<Page<ItemDto>> getFilteredItems(
            ...
            @Parameter(description = "Filter expiring soon items") @RequestParam(required = false) Boolean expiringSoon,
            @Parameter(description = "Days for expiry filter") @RequestParam(required = false, defaultValue = "7") Integer expiryDays,
            @Parameter(description = "Sort by field") @RequestParam(required = false, defaultValue = "created") String sortBy,
            @Parameter(description = "Page number") @RequestParam(defaultValue = "0") int page,
            @Parameter(description = "Page size") @RequestParam(defaultValue = "20") int size,
            Authentication authentication) {
        
        logger.info("GET /api/items/filter - Fetching filtered items for user: {}", authentication.getName());
        
        Pageable pageable = PageRequest.of(page, size);
        Page<ItemDto> items = itemService.getItemsWithFilters(
            authentication.getName(), categoryId, search, expiringSoon, expiryDays, sortBy, pageable
        );
